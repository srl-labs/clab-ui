...
interface WindowHostOptions {
  ...
  topoViewer?: ClabUiHost["topoViewer"];
...
function createWindowClabUiHost(options: WindowHostOptions = {}): ClabUiHost {
  const targetWindow = options.targetWindow ?? window;
  const resolvedVsCodeApi =
    options.vscodeApi ??
    ((targetWindow as Window & { vscode?: WindowVsCodeApiLike }).vscode ?? undefined);
  const postMessage =
  ...
  return {
    ...
    meta: {
      isDevMock:
        isRecord(resolvedVsCodeApi) && Reflect.get(resolvedVsCodeApi, "__isDevMock__") === true,
      disableDevMockTraffic:
        isRecord(resolvedVsCodeApi) &&
        Reflect.get(resolvedVsCodeApi, "__disableDevMockTraffic__") === true
    },
    explorer,
    topoViewer,
    topology
