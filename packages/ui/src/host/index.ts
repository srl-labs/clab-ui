...
function resolveWindow(): Window {
  if (typeof window === "undefined") {
    throw new Error("Window is unavailable in this runtime");
  }
  return window;
}
...
function getWindowVsCodeApi(targetWindow: Window): WindowVsCodeApiLike | undefined {
  const value = (targetWindow as Window & { vscode?: unknown }).vscode;
  if (!isRecord(value) || typeof value.postMessage !== "function") {
    return undefined;
  }
  return {
    postMessage: (message: unknown) => value.postMessage(message)
  };
}

function acquireWindowVsCodeApi(targetWindow: Window): WindowVsCodeApiLike | undefined {
  const maybeAcquire = (
  ...
  try {
    ...
    if (!isRecord(api) || typeof api.postMessage !== "function") {
      return undefined;
    }
    const resolvedApi = api as unknown as WindowVsCodeApiLike;

    // Preserve the historic window.vscode convention for callers that still read it.
    (targetWindow as Window & { vscode?: WindowVsCodeApiLike }).vscode = resolvedApi;
    return {
      postMessage: (message: unknown) => resolvedApi.postMessage(message)
    };
  } catch {
    return undefined;
  }
...
function hasSnapshotModeAndState(value: Record<string, unknown>): boolean {
  return (
    (value.mode === "edit" || value.mode === "view") &&
    (value.deploymentState === "deployed" ||
      value.deploymentState === "undeployed" ||
      value.deploymentState === "unknown")
  );
}
...
function hasValidTopologyMessageEnvelope(value: Record<string, unknown>): value is Record<
  string,
  unknown
> & {
  type: TopologyHostMessageType;
...
function isTopologyHostAckPayload(value: Record<string, unknown>): boolean {
  return (
...
function isTopologyHostRejectPayload(value: Record<string, unknown>): boolean {
  return (
    typeof value.revision === "number" &&
    Number.isFinite(value.revision) &&
    value.reason === "stale" &&
...
function isTopologyHostResponseMessage(value: unknown): value is TopologyHostResponseMessage {
  if (!isRecord(value) || !hasValidTopologyMessageEnvelope(value)) {
    return false;
  }
  ...
  switch (value.type) {
    ...
    case "topology-host:ack":
      return isTopologyHostAckPayload(value);
    case "topology-host:reject":
      return isTopologyHostRejectPayload(value);
...
export function createWindowClabUiHost(options: WindowHostOptions = {}): ClabUiHost {
  const targetWindow = options.targetWindow ?? resolveWindow();
  let resolvedVsCodeApi =
    options.vscodeApi ?? getWindowVsCodeApi(targetWindow) ?? acquireWindowVsCodeApi(targetWindow);
  const postMessage =
    options.postMessage ??
    ((message: unknown) => {
      resolvedVsCodeApi ??= getWindowVsCodeApi(targetWindow) ?? acquireWindowVsCodeApi(targetWindow);
      resolvedVsCodeApi?.postMessage(message);
    });
      ...
      const ensureListener = (): void => {
        ...
        subscribe((event) => {
          ...
          if (event.data.type === "topology-host:snapshot") {
            if (request.expectedType !== "snapshot") {
              request.reject(new Error("Unexpected snapshot response"));
              pending.delete(requestId);
              return;
            }
            if (!isTopologySnapshot(event.data.snapshot)) {
              request.reject(new Error("Snapshot message missing payload"));
              pending.delete(requestId);
              return;
            }
          ...
          if (request.expectedType !== "command") {
            request.reject(new Error("Unexpected command response"));
            pending.delete(requestId);
            return;
          }
          if (!isTopologyHostResponseMessage(event.data)) {
            request.reject(new Error("Invalid command response payload"));
            pending.delete(requestId);
            return;
          }
...
export function createApiClabUiHost(options: ApiHostOptions = {}): ClabUiHost {
  ...
  return {
    ...
    explorer: options.explorer ?? baseHost.explorer,
    topoViewer: options.topoViewer ?? baseHost.topoViewer
  };
}
...
export function getClabUiHost(): ClabUiHost {
  if (!currentHost) {
    currentHost = createWindowClabUiHost();
  }
  return currentHost;
}
