...
function provider(roots: unknown[]): {
...
test("buildExplorerSnapshot hides configured node and toolbar commands", async () => {
  ...
  const runningSection = snapshot.sections.find(
    (section) => section.id === "runningLabs",
  );
  assert.ok(runningSection);
  assert.equal(
    runningSection.toolbarActions.some(
      (action) => action.commandId === "containerlab.inspectAll",
    ),
    false,
  );
  ...
  assert.equal(
    endpointNode.actions.some(
      (action) => action.commandId === "containerlab.install.edgeshark",
    ),
    false,
  );
  assert.equal(
    endpointNode.actions.some(
      (action) => action.commandId === "containerlab.uninstall.edgeshark",
    ),
    true,
  );
...
test("buildExplorerSnapshot respects per-node supported command ids", async () => {
  const localBackend = {
  ...
  const providers = {
    runningProvider: provider([localBackend]),
    localProvider: provider([]),
    helpProvider: provider([]),
  } as ExplorerSnapshotProviders;
  ...
  const actions = snapshot.sections[0].nodes[0].actions.map(
  ...
  assert.equal(snapshot.sections[0].nodes[0].statusIndicator, undefined);
  assert.equal(snapshot.sections[0].nodes[0].primaryAction, undefined);
});

test("API endpoint status is binary for icon-only connected state", async () => {
  const providers = {
  ...
  assert.deepEqual(
    snapshot.sections[0].nodes.map((node) => node.statusIndicator),
    ["green", "red", "red"],
  );
});
...
test("buildExplorerSnapshot disables running-only actions for stopped containers", async () => {
  ...
  assert.equal(
    actionByCommand("containerlab.node.attachShell")?.disabled,
    true,
  );
  ...
  assert.equal(
    actionByCommand("containerlab.node.openBrowser")?.disabled,
    true,
  );
  ...
  assert.equal(
    actionByCommand("containerlab.node.manageImpairments")?.disabled,
    true,
  );
...
test("buildExplorerSnapshot supports opt-in lazy file explorer section", async () => {
  ...
  const collapsedRootCommands = collapsedRoot.actions.map(
    (action) => action.commandId,
  );
...
test("buildExplorerSnapshot applies host-contributed endpoint, lab, file, and toolbar actions", async () => {
  ...
  assert.ok(runningSection);
  assert.equal(
    runningSection.nodes[0].actions.some(
      (action) => action.commandId === "host.endpoint.import",
    ),
    true,
  );
  assert.equal(
    runningSection.nodes[0].children[0].actions.some(
      (action) => action.commandId === "host.lab.export",
    ),
    true,
  );
  ...
  assert.equal(
    actionBindings.get(addEndpointAction.actionRef)?.commandId,
    "host.section.addEndpoint",
  );
  ...
  assert.equal(
    fileSection.toolbarActions.some(
      (action) => action.commandId === "host.file.import",
    ),
    true,
  );
  assert.equal(
