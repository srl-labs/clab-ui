...
export const test = base.extend<{ topoViewerPage: TopoViewerPage }>({
  topoViewerPage: async ({ page }, use, testInfo: TestInfo) => {
    ...
    const ensureDevApiReady = async (): Promise<void> => {
      if (!page.url().startsWith(API_BASE_URL)) {
    ...
    const topoViewerPage: TopoViewerPage = {
      ...
      listTopologyFiles: async () => {
        const response = await browserRequest(withSession("/files"));
        if (!response.ok) {
      ...
      resetFiles: async () => {
        const response = await browserRequest(withSession("/api/reset"), { method: "POST" });
        if (!response.ok) {
          throw new Error(`Failed to reset files: ${response.statusText}`);
        }
      ...
      writeYamlFile: async (filename: string, content: string) => {
        // Build full path to YAML file
        const yamlPath = path.join(TOPOLOGIES_DIR, filename);
        const response = await browserRequest(withSession(`/file/${encodeURIComponent(yamlPath)}`), {
          method: "PUT",
          body: content,
      ...
      writeAnnotationsFile: async (filename: string, content: object) => {
        // Build full path to annotations file
        const annotationsPath = path.join(TOPOLOGIES_DIR, `${filename}.annotations.json`);
        const response = await browserRequest(withSession(`/file/${encodeURIComponent(annotationsPath)}`), {
          method: "PUT",
