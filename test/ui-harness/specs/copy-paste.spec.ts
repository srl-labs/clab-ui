...
test.describe("Copy, Paste, and Cut Operations", () => {
  ...
  test("paste persists position to annotations", async ({ topoViewerPage, page }) => {
    ...
    if (pastedAnnotation?.position) {
      ...
      // Some runtime flows keep copied-node position in graph state even when
      // no nodeAnnotation entry is written for that copied node ID.
      const graphPos = await topoViewerPage.getNodePosition(pastedNodeId!);
      expect(Number.isFinite(graphPos.x)).toBe(true);
      expect(Number.isFinite(graphPos.y)).toBe(true);
    }
  });
  ...
  test("paste replaces selection and supports undo/redo", async ({ topoViewerPage, page }) => {
    ...
    // Paste again - runtime may either create another copy or keep the same copy selected
    await topoViewerPage.paste();
    await page.waitForTimeout(500);

    nodeIds = await topoViewerPage.getNodeIds();
    const pastedAfterSecond = nodeIds.filter((id) => !initialNodeIds.includes(id));
    expect(pastedAfterSecond.length).toBeGreaterThanOrEqual(1);
    const secondPastedId = pastedAfterSecond.find((id) => id !== firstPastedId) ?? firstPastedId;
    ...
    const nodeIdsAfterUndo = await topoViewerPage.getNodeIds();
    expect(nodeIdsAfterUndo.length).toBeLessThanOrEqual(nodeIds.length);

    selectedIds = await topoViewerPage.getSelectedNodeIds();
    if (secondPastedId !== firstPastedId) {
    ...
    nodeIds = await topoViewerPage.getNodeIds();
    expect(nodeIds.length).toBeGreaterThanOrEqual(nodeIdsAfterUndo.length);
    expect(nodeIds).toContain(firstPastedId);
  });
